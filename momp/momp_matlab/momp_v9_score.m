...
%This version is used in "Section VI.E : MOMP as an Anytime Algorithm"
%In addition to all experiments it returns the convergence score vs. time.
%For more info please refere to Section VI.E of the paper
...
%% Output:
            ...
            % scores (convergence score 0 to 1)
...
function [momp_out, momp_loc,scores] = momp_v9_score(T, m, verbose, run_mp, plotting)
...
function [pruned_T, pruned_indices] = prune(T, m, indices, uamp, bsf)
    ...
    if bsf < min(uamp), error('Prune thresholds error!\n'); end
